...
const generateToken = payload => {
    return new Promise(resolve =>{
        const token = jwt.sign(payload, process.env.TOKEN_SECRET, {expiresIn: '10s'});
        resolve(token);
    });
}
...
const generateRefreshToken = payload => {
    return new Promise(resolve => {
        const refreshToken = jwt.sign(payload, process.env.REFRESH_TOKEN_SECRET, {expiresIn: '90 days'});
...
const decodeToken = (token, isRefreshToken) => {
    return new Promise((resolve, reject) => {
        jwt.verify(token, isRefreshToken ? process.env.REFRESH_TOKEN_SECRET : process.env.TOKEN_SECRET, (error, decoded) => {
            if (error) {
                console.log(error); 
                reject(error);
            }
...
const validate = (authorization, isRefreshToken)  => {
    return new Promise((resolve, reject) => {
        if(!authorization) return reject(new Error('Missing Authorization header'));
        const headerParts = authorization.split(' ');
        if(headerParts.length < 2) return reject(new Error('Invalid Authorization header'));
        const token = headerParts[1].toString();
        decodeToken(token, isRefreshToken).then(result => resolve(result)).catch(err => {reject(err); console.warn('failed to decode')});
    });
};
...
const reAuthWithExpiredAccessToken = async (expiredAccessToken) => {
    return new Promise(async (resolve, reject) => {
        ...
        const refreshTokenObjectFromDB = await RefreshTokenModel.getById(expiredAccessToken);
        const { refresh_token, is_used, is_revoked, expiry_date } = refreshTokenObjectFromDB;
        ...
        const loggedInUser = await User.findOne({_id: userId}).select('username role email password');
...
const validationMiddleware = async (req, res, next) => {
    ...
    if(expiredAccessToken) {
        ...
        try {
            ...
            const newAuthInfo = await reAuthWithExpiredAccessToken(expiredAccessToken);
            console.info('refresh token successful', newAuthInfo);

            const { token, payload } = newAuthInfo;

            res.header('Access-Control-Expose-Headers', 'Shop-Refreshed-Access-Token');
