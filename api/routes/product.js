  ...
  Product.find()
    ...
    .then((docs) => {
      ...
      docs.length
        ...
        : res.status(200).json({
            message: "No products exists as yet. Create our first one!",
          });
    })
    .catch((err) => {
      console.log(err);
      ...
      res.status(200).json({
        ...
        request: {
          ...
          body: {
            name: "String",
            price: "Number",
            productImage: "image/jpeg or image/png",
          },
        },
      });
    })
...
router.put("/:id/unavailable", (req, res) => {
  ...
  const { update } = req.body;
  Product.findByIdAndUpdate({ _id }, { available: false })
    .exec()
    .then((result) => {
      console.log(result);
...
router.put("/:id/available", (req, res) => {
  const _id = req.params.id;
  const { update } = req.body;
  Product.findByIdAndUpdate({ _id }, { available: true })
    .exec()
    .then((result) => {
      console.log(result);
      res.status(204).send(result);
